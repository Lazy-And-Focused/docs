...
## Вложенные в дочернюю "page" [#child-page-subs]
...
>
> Названия папок, обёртонные в `()`, являются группой. То есть, явно
> группируют вложенные каталоги и файлы, не влияя на путь (URL) приложения.


- `(pages)`

  > Вложенные (дочерние) страницы родительской страницы.
...
- `_api`
  ...
  >
  > \*_Обычно выносится в `~/src/shared/` папку._

- `_lib`

  > Библиотечный код, который нужен другим модулям в сегменте... Не
  > путайте с общей логикой, которая используется повсеместно.
